...
///
/// 本番でTikTok Liveと連携する際は、この抽象を実装するクラス
/// （例: `TikTokLiveCommentService`）に差し替えるだけでよく、
/// ゲーム側（[LiveGameScreen] 以下）のロジックは変更不要になるように設計している。
abstract class LiveCommentService {
  /// 投票キーワード（例: "1", "2"）が流れてくるストリーム。
  Stream<String> get voteStream;

  /// このラウンドで有効な投票キーワードを更新する。
  /// TikTok Live実装では、有効なキーワード以外のコメントを無視するために使う。
...
/// 実際のTikTok Live API連携が未実装の間、動作確認・デモ配信用に使う
